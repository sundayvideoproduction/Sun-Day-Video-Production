...
async function initApp() {
    showLoading('Initializing high performance platform...');
...
function setupEventListeners() {
    ...
    downloadedBtn.addEventListener('click', () => showScreen(3));
    backButton.addEventListener('click', handleBackButton);
    backToMain.addEventListener('click', () => showScreen(1));
    backToMainFromDownloaded.addEventListener('click', () => showScreen(1));
...
async function loadVideos() {
    try {
        if (navigator.onLine) {
            ...
            if (cachedVideos) videos = JSON.parse(cachedVideos);
            if (cachedStories) stories = JSON.parse(cachedStories);
...
async function loadPasswords() {
    try {
        if (navigator.onLine) {
            ...
            const cached = localStorage.getItem(SECURITY.STORAGE_PREFIX + 'passwords');
            if (cached) passwords = JSON.parse(cached);
        }
    } catch (error) {
...
async function loadContactLinks() {
    try {
        if (navigator.onLine) {
            ...
            const cached = localStorage.getItem(SECURITY.STORAGE_PREFIX + 'contacts');
            if (cached) contactLinks = JSON.parse(cached);
        }
    } catch (error) {
    ...
    videosToShow.forEach(video => {
        ...
        videoItem.innerHTML = `
            <div class="video-info">
                <div class="video-name">${video.name} ${isPreloaded ? '<i class="fas fa-bolt" style="color: #00ff00; margin-left: 5px; font-size: 10px;"></i>' : ''}</div>
...
function renderDownloadedList() {
    ...
    if (videoIds.length === 0) {
        downloadedList.innerHTML = `
            <div style="text-align: center; padding: 20px; color: #8a93a7;">
                <i class="fas fa-download" style="font-size: 24px; margin-bottom: 10px;"></i>
                <div>No downloaded videos</div>
                <div style="font-size: 10px; margin-top: 5px;">
                    Download videos when online to watch offline
                </div>
            </div>
        `;
        return;
    ...
    videoIds.forEach(videoId => {
        ...
        videoItem.innerHTML = `
            <div class="video-info">
                ...
                <div class="video-meta">
                    ${videoInfo.type} ${videoInfo.story ? `• ${videoInfo.story}` : ''}
        ...
        // Add event listeners
        videoItem.querySelector('[data-action="play"]').addEventListener('click', (e) => {
            e.stopPropagation();
            playDownloadedVideo(videoId);
        });
        
        videoItem.querySelector('[data-action="delete"]').addEventListener('click', (e) => {
            e.stopPropagation();
            if (confirm('Delete this downloaded video?')) {
                deleteDownloadedVideo(videoId);
                renderDownloadedList();
...
function startVideoPreloading() {
    ...
    if (cloudinaryVideos.length === 0) {
        preloadText.textContent = 'No videos to preload';
        setTimeout(() => {
    ...
    preloadText.textContent = `Preloading ${cloudinaryVideos.length} videos...`;
...
function preloadNextVideo() {
    if (videoPreloadQueue.length === 0) {
        preloadText.textContent = 'All videos preloaded';
    ...
    if (!preloadedVideos.has(video.id)) {
        ...
        preloadText.textContent = `Preloaded ${percent}%`;
